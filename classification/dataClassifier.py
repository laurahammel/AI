...
def enhancedPacmanFeatures(state, action):
    ...
    for i in range(width):
        for j in range(height):
            if closestFoodPos == None or (food[i][j] and manhattanDist((i,j), pacmanPosition) < manhattanDist(closestFoodPos,pacmanPosition)):
                closestFoodPos = (i,j)
    """

    closestGhostPos = ghostPositions[0]

    for i in range(len(ghostPositions)):
        if util.manhattanDistance(ghostPositions[i],pacmanPosition) < util.manhattanDistance(closestGhostPos, pacmanPosition):
            closestGhostPos = ghostPositions[i]

    features['closestGhost'] = (width + height - util.manhattanDistance(pacmanPosition, closestGhostPos))
...
def readCommand( argv ):
    ...
    parser.add_option('-c', '--classifier', help=default('The type of classifier'), choices=['mostFrequent', 'nb', 'naiveBayes', 'perceptron', 'minicontest'], default='mostFrequent')
