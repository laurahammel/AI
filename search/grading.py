...
class Grades:
  ...
  def __init__(self, projectName, questionsAndMaxesList, edxOutput=False, muteOutput=False, autolabOutput=False):
    """
...
"""
    ...
    ''' Print AutoLab grade string'''
    if self.autolabOutput:
        print '{ "scores": { "%s": %d' % (self.questions[0], self.points[self.questions[0]]),
        for q in self.questions[1:]:
            print ', "%s": %d' % (q, self.points[q]),

        competitionScore = 2
        print '}, "scoreboard":["substituteHere"] }'
    #print '}, "scoreboard":["', scoreboardName, '", "', competitionScore, '"] }'
